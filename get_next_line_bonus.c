...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/08/01 20:59:20 by dosokin           #+#    #+#             */
/*   Updated: 2024/08/04 14:18:33 by dosokin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || fd >= 1024)
		return (NULL);
	if (!stack[fd])
		stack[fd] = ft_calloc(1, sizeof(char));
	while (stack[fd])
	{
		i = find_new_line(stack[fd]);
		if (i != -1)
			return (return_line(&stack[fd], i));
		r = read_file_and_push_to_stack(&stack[fd], fd);
		if (r > 0)
			continue ;
		if (r < 0)
			break ;
		return (end_of_process(&stack[fd], EOF));
	}
	return (end_of_process(&stack[fd], ERR));
}
